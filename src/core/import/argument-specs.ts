...
// A variable whose name suggests a credential — masked as a `secret` field and
// never carries a default (council §5). Heuristic, deliberately broad; mirrors
// the template reader's `looksLikeSecretName` (kept local so this pure core
// module has no dependency on the reader page).
